...
# a better version of this would be one that stores paths for profiles
# automatically
...
def tags_collection(user, account_ids):
    ...
    container_paths = [j["path"] for i in containers for j in i]
    workspaces = [user.workspaces(i)["workspace"] for i in container_paths]
    workspaces = [i for sublist in workspaces for i in sublist]

    # we get a quota exceeded because of the looping; need a better choice
    # we also get it when trying to get the ["tag"] array from the response
    # workspace_paths = [i["path"] for i in workspaces]
    ...
    return user.tags("accounts/87114/containers/1804338/workspaces/12")
